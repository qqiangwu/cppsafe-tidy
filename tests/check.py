...
def get_options(checkfile):
    options = Options()

    with open(checkfile) as f:
        line = f.readline()
        if 'CHECK=' not in line:
            raise RuntimeError('invalid test file, specify check via CHECK=')
        for opt in (l.strip() for l in line.split() if l.strip()):
            fields = opt.split('=')
            if len(fields) != 2 or any(len(f) == 0 for f in fields):
                continue
...
def get_module_path():
    if os.path.exists('../build/debug/libcppsafe-tidy.so'):
        return '../build/debug/libcppsafe-tidy.so'
    else:
        return '../build/libcppsafe-tidy.so'
...
class CheckRunner:
    def __init__(self, args, extra_args):
        opts = get_options(args.input_file_name)

        self.resource_dir = args.resource_dir
        self.assume_file_name = args.assume_filename
        self.input_file_name = args.input_file_name
        self.check_name = opts.check
        self.temp_file_name = '/tmp/cppsafe-tidy-{}'.format(self.input_file_name)
        ...
        if not any(
            ...
            if opts.options:
                import json
                self.clang_tidy_extra_args.append("--config={{\"CheckOptions\":{}}}".format(json.dumps(opts.options, separators=(',', ':'))))
            else:
...
def update_env():
    with open('../build/CMakeCache.txt') as f:
        lines = [line for line in f if line.startswith('LOCAL_CLANG')]
        if len(lines) == 0:
            raise RuntimeError('LOCAL_CLANG cannot be infered')

        path = os.path.join(lines[0].split('=')[1].strip(), 'bin')
